...
def reduce_neighbours(elements, neighbours, hit):
    value_of_hit = elements[hit]

    for neighbour in neighbours:
        elements[neighbour] = max(0, elements[neighbour] - value_of_hit)
        print(elements[neighbour])
    return elements
...
def matrix_bombing_plan(m):
    ...
    for elem in elements:
        reduced_elements = reduce_neighbours(
            elements, find_neighbours(m, elem), elem)
        sum_all_values = sum([reduced_elements[reduced_elem]
                              for reduced_elem in reduced_elements])
        print(elem, sum_all_values)
        result[elem] = sum_all_values
